...
export default function RiwayatPesanan({ transactions, setTransactions }) {
  ...
  const handleBulkDelete = () => {
    ...
    Swal.fire({
      title: 'Hapus Riwayat?',
      text: `${selectedIds.length} riwayat pesanan akan dihapus secara permanen.`,
      icon: 'warning',
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: 'Hapus Riwayat?',
      text: `Riwayat pesanan ${id} akan dihapus secara permanen.`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: 'var(--red)',
      cancelButtonColor: 'var(--border-2)',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} style={{ padding: '48px 16px', textAlign: 'center', color: 'var(--text-3)', fontSize: 13 }}>
                    {transactions.filter(t => t.status === 'Diambil').length === 0 ? 'Belum ada riwayat pesanan' : 'Tidak ada riwayat yang sesuai filter'}
                  </td>
                </tr>
              ) : (
                filtered.map((t, idx) => (
        ...
        <div style={{
          ...
          <span>Menampilkan <strong style={{ color: 'var(--text)' }}>{filtered.length}</strong> riwayat pesanan</span>
